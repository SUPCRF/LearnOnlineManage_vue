    ...
    <div class="dormitory">
      <div class="searchWord">
        <el-button type="primary" class="add-btn" @click="addFormVisible = true"
          >添加考试</el-button
        >
        <el-dialog title="添加考试" :visible.sync="addFormVisible">
          <el-form :model="form">
            ...
            <el-form-item label="考试答案" :label-width="formLabelWidth">
              <el-input
                type="textarea"
                v-model="form.answer"
                autocomplete="off"
...
export default class Index extends Vue {
  ...
  getInfo() {
    ...
    axios({
      url: "http://localhost:8000/api/exam/all",
      method: "get",
  ...
  add() {
    ...
    axios({
      url: "http://localhost:8000/api/exam-content/save",
      method: "post",
      data: formdata,
      headers: { Authorization: token },
    }).then((res) => {
  ...
  handleStart(index, row) {
    this.$confirm("是否恢复？", "确定恢复", {
      ...
      .then(() => {
        ...
        console.log(formdata);
        axios({
          url: "http://localhost:8000/api/exam/start",
          method: "post",
          data: formdata,
          headers: { Authorization: token },
        })
  ...
  handleDelete(index, row) {
    this.$confirm("是否删除？", "确定删除", {
      ...
      .then(() => {
        ...
        console.log(formdata);
        axios({
          url: "http://localhost:8000/api/exam/delete",
          method: "post",
          data: formdata,
          headers: { Authorization: token },
        })
